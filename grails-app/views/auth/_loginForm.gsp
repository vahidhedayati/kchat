...
<g:formRemote name="loginForm" url="[controller:'auth',action:'signIn']" update="${did ?: 'authBox' }" onSuccess="redirSite()">
...
function redirSite() {
	...
	console.log('LURL : '+lastUrl+' cuser: '+cuser)
	if (lastUrl) { 
...
<g:select id="ldapauth" class="form-control" name="authProvider" from="${kchat.AuthProvider.list()}" optionKey="name" optionValue="friendlyName" required="" value="${fieldValue(bean:loginCmd, field:'authProvider')}"/>
<div class="clearall"></div>
<input type="hidden" name="targetUri" value="${session.lastPlace ?: session.lastURL ?: curl ?: '/'  } ">
<br/>
